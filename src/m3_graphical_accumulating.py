...
def draw_parallel_lines(n, point, length, window):
    ...
    for k in range(n):
        ...
        line_k = rg.Line(rg.Point(point.x,point.y+(30*k)),rg.Point(end_point.x,end_point.y+(30*k)))
        line_k.attach_to(window)
...
def draw_lines(n, point, window):
    ...
    for k in range(n):
        end_point = rg.Point(point.x + 100, point.y - 100 +   ((200 / n) * (k + 1)))
        line_k = rg.Line(rg.Point(point.x,point.y),end_point)
        line_k.attach_to(window)
